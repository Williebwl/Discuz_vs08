...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        /// </summary>
        /// <param name="aId">附件id</param>
        public int UpdateAttachmentviewnum(int aid)
        {
            return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}updateattachmentviewnum", BaseConfigs.GetTablePrefix),
                                       DbHelper.MakeInParam("@aid", (DbType)SqlDbType.Int, 4, aid));
        }
        /// <summary>
        /// 美图投票
        ...
        /// <returns></returns>
        public int UpdatePicVote(int aid, string comment)
        {
            DbParameter[] parms = {
                                    DbHelper.MakeInParam("@Aid", (DbType)SqlDbType.Int, 4, aid),
                                    DbHelper.MakeInParam("@Comment", (DbType)SqlDbType.NVarChar,30,comment),
                                    DbHelper.MakeInParam("@Result", (DbType)SqlDbType.Int,4,null),
                                  };
            parms[2].Direction = ParameterDirection.Output;
            DbHelper.ExecuteDataset(CommandType.StoredProcedure, string.Format("{0}updatepicvote", BaseConfigs.GetTablePrefix), parms);
        ...
        /// <returns></returns>
        public DataSet GetVotelist(int aid, string posttitle, int type, int pageIndex, int pageSize, string ordertype, ref int total)
        {
            DbParameter[] parms = { 
                                  DbHelper.MakeInParam("@Aid",(DbType)SqlDbType.Int,4,aid),     
                                  DbHelper.MakeInParam("@Title",(DbType)SqlDbType.NVarChar,60,posttitle),     
                                  DbHelper.MakeInParam("@Type",(DbType)SqlDbType.Int,4,type),
                                  DbHelper.MakeInParam("@PageIndex",(DbType)SqlDbType.Int,4,pageIndex),
                                  DbHelper.MakeInParam("@PageSize",(DbType)SqlDbType.Int,4,pageSize),
                                  DbHelper.MakeInParam("@OrderType",(DbType)SqlDbType.VarChar,5,ordertype),
                                  DbHelper.MakeInParam("@Total",(DbType)SqlDbType.Int,4,total),
                                  };
            parms[6].Direction = ParameterDirection.Output;
            DataSet ds = DbHelper.ExecuteDataset(CommandType.StoredProcedure,
                                          string.Format("{0}getVotelist",
                                          BaseConfigs.GetTablePrefix), parms);
            if (parms[6].Value != null)
            {
                total = int.Parse(parms[6].Value.ToString());
            }
            return ds;
        }
        /// <summary>
        ...
        /// <param name="total"></param>
        /// <returns></returns>
        public DataSet GetVotelistByUid(int aid, int uid, int type, int pageIndex, int pageSize, string ordertype, ref int total)
        {
            DbParameter[] parms = { 
                                  DbHelper.MakeInParam("@Aid",(DbType)SqlDbType.Int,4,aid),     
                                  DbHelper.MakeInParam("@Uid",(DbType)SqlDbType.Int,4,uid),     
            ...
            parms[6].Direction = ParameterDirection.Output;
            DataSet ds = DbHelper.ExecuteDataset(CommandType.StoredProcedure,
                                          string.Format("{0}getVotelistByUid",
                                          BaseConfigs.GetTablePrefix), parms);
            if (parms[6].Value != null)
            {
                total = int.Parse(parms[6].Value.ToString());
