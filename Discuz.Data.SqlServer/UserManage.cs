...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public IDataReader CheckUserName(string userName)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@username",(DbType)SqlDbType.Char,20, userName)
								   };
            return DbHelper.ExecuteReader(CommandType.StoredProcedure, string.Format("{0}checkusername", BaseConfigs.GetTablePrefix), parms);
        }
    ...
    UPDATE [{0}users] SET [credits] = {1} WHERE [uid] = @uid", BaseConfigs.GetTablePrefix, creditExpression);
        ...
        /// <returns></returns>
        public int submitlotteryinfo(int userid , string ticketnumber,ref int result) {
            DbParameter[] parms = 
			{
               
				DbHelper.MakeInParam("@userid", (DbType)SqlDbType.Int,4, userid),
				DbHelper.MakeInParam("@ticketnumber", (DbType)SqlDbType.NVarChar,50,ticketnumber),
                DbHelper .MakeOutParam("@result",(DbType)SqlDbType .Int,4)
			};

            int total= DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}userlotteryinsert",BaseConfigs.GetTablePrefix), parms);
            if (parms[2].Value != null) {
                result = Convert.ToInt32(parms[2].Value);
            }
        ...
        public DataTable GetUserLotteryList(int pagesize,int pageindex,ref int total) {
            //if (pagesize != 0 && pageindex != 0)
            //{
            //    string commandText = string.Format("SELECT * FROM (SELECT  ROW_NUMBER() OVER (ORDER BY Lotteryid)as RowIndex,[phone],[ticketnumber],[lotterdate],[Memo] FROM [{0}userLottery] WHERE [ISWinning] =1)as temp  where RowIndex >({1} - 1) * {2} and RowIndex <={1} * {2}",
            //                                         BaseConfigs.GetTablePrefix, pageindex, pagesize);
            //    return DbHelper.ExecuteDatasetInMasterDB(CommandType.Text, commandText).Tables[0];
            //}
            //else {
            //    string commandText = string.Format("SELECT *   FROM [{0}userLottery] WHERE [ISWinning] =1",BaseConfigs.GetTablePrefix);
            //    return DbHelper.ExecuteDatasetInMasterDB(CommandType.Text, commandText).Tables[0];
            //}
            ...
            DbParameter[] parms ={
                                 DbHelper .MakeInParam ("@PageSize",(DbType)SqlDbType .Int,4,pagesize),
                                 DbHelper .MakeInParam ("@PageIndex",(DbType )SqlDbType .Int,4,pageindex),
                                 DbHelper .MakeOutParam ("@Total",(DbType)SqlDbType .Int,4)
                                 };

            DataTable dt = DbHelper.ExecuteDataset(CommandType.StoredProcedure, string.Format("{0}userlottrryList", BaseConfigs.GetTablePrefix), parms).Tables[0];
           if (parms[2].Value != null) {
        ...
        public DataTable GetuserRealtimeList() {
            DataTable dt = DbHelper.ExecuteDataset(CommandType.StoredProcedure, string.Format("{0}userRealtimeList", BaseConfigs.GetTablePrefix), null).Tables[0];
            return dt;
        }
