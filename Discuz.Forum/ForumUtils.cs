...
namespace Discuz.Forum
{
    ...
    public class ForumUtils
    {
        ...
        public static void WriteUserCookie(UserInfo userinfo, int expires, string passwordkey, int templateid, int invisible)
        {
            ...
            //cookie新增的键值对
            //开始
            cookie.Values["username"] = HttpUtility.UrlEncode(userinfo.Username, Encoding.GetEncoding("utf-8"));
            cookie.Values["email"] = HttpUtility.UrlEncode(userinfo.Email, Encoding.GetEncoding("utf-8"));
            cookie.Values["mobile"] = HttpUtility.UrlEncode(userinfo.Mobile, Encoding.GetEncoding("utf-8"));
            //结束
        ...
        public static string ConvertPhone(string phone) {
            ...
            if (!string.IsNullOrEmpty(phone)) {
                ...
                str = phone.Replace(phone.Substring(3, 4).ToString(), "****");
            }
        ...
        public static string ConvertDay(string name, string date)
        {
            ...
            else
            {
                ...
                string str_day = string.Format("{0:M}", dt);
                if (ts.Minutes.ToString() == "0")
                {
                    str_msg = name + "   " + "刚刚参与了" + str_day + "的抽奖活动";
                }
                else
                {
                    str_msg= name + "   " + ts.Minutes.ToString() + "分钟前 参与了" + str_day + "的抽奖活动";
                }
            }
